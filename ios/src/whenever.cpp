...
void whenever(char *path,char *returnPath)
{
    ...
    Mat result;
    cvtColor(image, result, COLOR_BGR2Lab);
    Mat lab[3];
    split(result, lab);

    Scalar aMean = mean(lab[1]);
    float aAvg = aMean.val[0];

    Scalar bMean = mean(lab[2]);
    float bAvg = bMean.val[0];

    lab[1] = lab[1] -
             ((aAvg - 128) * (lab[0] / 255.0) * 1.1);

    lab[2] = lab[2] -
             ((bAvg - 128) * (lab[0] / 255.0) * 1.1);

    merge(lab, 3, result);

    cvtColor(result, image, COLOR_Lab2BGR);
    ...
    bgr[0] = (bgr[0] - vmin_b) * 255 / (vmax_b - vmin_b) * 1.1;
    bgr[1] = (bgr[1] - vmin_g) * 255 / (vmax_g - vmin_g) * 1.1;
    bgr[2] = (bgr[2] - vmin_r) * 255 / (vmax_r - vmin_r) * 1.2;

    Mat output;
    merge(bgr, 3, output);
    imwrite(returnPath, output);
}
